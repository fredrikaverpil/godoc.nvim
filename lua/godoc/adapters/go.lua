...
local function get_std_packages_golist()
  ...
  if vim.v.shell_error ~= 0 then
    vim.notify(
      "Failed to get package list using 'go list'",
...
local function get_std_packages_stdsym()
  ...
  if stdsym == 1 then
    ...
    if vim.v.shell_error ~= 0 then
      vim.notify(
        "Failed to get package list using stdsym, falling back on 'go list'",
        vim.log.levels.ERROR
      )
...
local function get_gomod_packages()
  ...
  if go_mod ~= "" then
    ...
    local mod_packages = vim.fn.systemlist(
      string.format("cd %s && go list -e all", vim.fn.shellescape(mod_dir))
    )
    if vim.v.shell_error ~= 0 then
      vim.notify(
        "Failed to get package list using 'go list'",
...
local function goto_definition(item, picker_gotodef_fun)
  ...
  if go_mod_dir == "" then
    vim.notify(
      "Failed to find go.mod file, can only gotodef on std lib",
      vim.log.levels.WARN
    )
  else
  ...
  while client_id == nil and maxretries >= 0 do
    for _, client in
      ipairs(vim.lsp.get_clients({ name = "gopls", bufnr = buf }))
    do
      client_id = client.id
      break
    end
