...
function M.validate_adapter(adapter)
  ...
  for _, field in ipairs(required_fields) do
    if type(adapter[field.name]) ~= field.type then
      return false,
        string.format(
          "Adapter must have a '%s' field of type '%s'",
          field.name,
          field.type
        )
    end
...
function M.override_adapter(default_adapter, user_opts)
  ...
  return {
    ...
    get_syntax_info = user_opts.get_syntax_info
      or default_adapter.get_syntax_info,
    goto_definition = user_opts.goto_definition
      or default_adapter.goto_definition,
